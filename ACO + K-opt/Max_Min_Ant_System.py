...
class MMAS(ACO):
    ...
    def __init__(self, graph, num_ants=10, alpha=1.0, beta=2.0, rho=0.1, q=1.0,
        ...
        self.pheromone = np.ones((self.num_cities, self.num_cities)) * self.max_pheromone
    ...
    def update_pheromones(self, routes, costs):
        ...
        for i in range(self.num_cities - 1):
            self.pheromone[best_route[i], best_route[i+1]] += self.q / best_cost
    ...
    def run(self, start_city=0, max_iterations=100):
        ...
        for iteration in range(max_iterations):
            ...
            if self.stagnation_count >= self.stagnation_limit:
                print(f"Reinicializando feromônios na iteração {iteration} por estagnação")
                self.pheromone = np.ones((self.num_cities, self.num_cities)) * self.max_pheromone
                self.stagnation_count = 0
